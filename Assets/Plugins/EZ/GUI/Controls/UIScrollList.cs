...
public class UIScrollList : MonoBehaviour, IUIObject
{
	...
	/// <summary>
	/// Adds extra space to the ends of the list
	...
	/// of performance by setting this set to false so
...
*/
	...
	protected void UpdateContentExtents(float change)
	{
		...
		ScrollListTo_Internal(Mathf.Clamp01((oldAmtOfPlay * scrollPos) / amtOfPlay));
	}
	...
	protected void PositionHorizontally(bool updateExtents)
	{
		...
		if(direction == DIRECTION.TtoB_LtoR)
		{
			...
			for (int i = 0; i < items.Count; ++i)
			{
				...
				items[i].transform.localPosition = new Vector3(edge - items[i].TopLeftEdge.x, GetYAlignment(items[i]));
		...
		else
		{
			...
			for (int i = 0; i < items.Count; ++i)
			{
				...
				items[i].transform.localPosition = new Vector3(edge - items[i].BottomRightEdge.x, GetYAlignment(items[i]));
				step = items[i].BottomRightEdge.x - items[i].TopLeftEdge.x + itemSpacingActual;
				contentExtents += step;
				edge -= step;
	...
	protected void PositionVertically(bool updateExtents)
	{
		...
		if(direction == DIRECTION.TtoB_LtoR)
		{
			...
			for (int i = 0; i < items.Count; ++i)
			{
				...
				items[i].transform.localPosition = new Vector3(GetXAlignment(items[i]), edge - items[i].TopLeftEdge.y);
		...
		else
		{
			...
			for (int i = 0; i < items.Count; ++i)
			{
				...
				items[i].transform.localPosition = new Vector3(GetXAlignment(items[i]), edge - items[i].BottomRightEdge.y);
				step = items[i].TopLeftEdge.y - items[i].BottomRightEdge.y + itemSpacingActual;
				contentExtents += step;
				edge += step;
...
*/
	...
	public void RemoveItem(int index, bool destroy)
	{
		...
		if (lastClickedControl != null &&
			(lastClickedControl == items[index] || lastClickedControl.Container.Equals(items[index])) )
