...
// 				else
	...
	protected void DoDragUpdate(POINTER_INFO curPtr)
	{
		...
		// Use the first object that isn't our dragging object:
		float nearest = float.PositiveInfinity;
		for (int i = 0; i < hits.Length; ++i)
		{
			if (hits[i].transform != dragObj.transform && hits[i].distance < nearest)
			{
				nearest = hits[i].distance;
				dragObj.DropTarget = hits[i].transform.gameObject;
